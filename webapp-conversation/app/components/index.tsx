    ...
    sendChatMessage(data, {
      ...
      async onCompleted(hasError?: boolean) {
        ...
        if (getConversationIdChangeBecauseOfNew()) {
          ...
          try {
            const firstQuestion = getChatList().find(item => item.isAnswer === false)?.content || ''
            ...
            await renameConversation(allConversations[0].id, title)
            const newAllConversations = produce(allConversations, (draft: any) => {
              draft[0].name = title
            })
            setConversationList(newAllConversations as any)
          } catch (err) {
            console.error('DeepSeek title error:', err)
  ...
  return (
    <div className='bg-white flex flex-col h-full overflow-hidden'>
      <Header
        ...
        onGoHome={() => handleConversationIdChange('-1')}
      />
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <div className='flex-1 flex flex-col overflow-hidden relative bg-white'>
          ...
          {hasSetInputs && (
            <div className='relative flex-1 w-full mx-auto max-w-[850px] flex flex-col overflow-hidden'>
              <Chat
                key={currConversationId}
                chatList={chatList}
                currConversationId={currConversationId}
                currInputs={currInputs as Record<string, any>}
                onFeedback={handleFeedback}
                visionConfig={visionConfig}
                fileConfig={fileConfig}
              >
                <div ref={chatListDomRef}>
                  <ConfigSence
                    conversationName={conversationName}
                    hasSetInputs={hasSetInputs}
                    isPublicVersion={isShowPrompt}
