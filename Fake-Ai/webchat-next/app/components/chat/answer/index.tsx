...
function parseThinkContent(raw: string): ThinkParseResult {
  const OPEN = '<think>'
  const CLOSE = '</think>'
  let rest = raw || ''
  let answer = ''
  ...
  while (rest) {
    ...
    if (closeIndex === -1) {
      thinkChunks.push(afterOpen)
      return {
        think: thinkChunks.join('\n\n').trim() || null,
        answer: answer.trim(),
        isThinking: true,
      }
    }

    thinkChunks.push(afterOpen.slice(0, closeIndex))
...
const Answer: FC<IAnswerProps> = ({
  ...
  const parsedContent = React.useMemo(() => parseThinkContent(content || ''), [content])
  const [isThinkOpen, setIsThinkOpen] = React.useState(true)
  ...
  React.useEffect(() => {
    if (parsedContent.think && parsedContent.isThinking) {
      setIsThinkOpen(true)
    }
  }, [parsedContent.think, parsedContent.isThinking])
  ...
  const agentModeAnswer = (
    <div>
      {agent_thoughts?.map((item, index) => (
        <div key={index}>
          {item.thought && (() => {
            const parsedThought = parseThinkContent(item.thought)
            return (
              <div>
                {parsedThought.think && (
                  <div className={s.thinkInline}>
                    <div className={s.thinkInlineTitle}>思考过程</div>
                    <StreamdownMarkdown content={parsedThought.think} />
                  </div>
                )}
                {parsedThought.answer && <StreamdownMarkdown content={parsedThought.answer} />}
              </div>
            )
          })()}
          {/* {item.tool} */}
                    ...
                    <div className="space-y-2">
                      {parsedContent.think && (
                        <div className={s.thinkBlock}>
                          <button className={s.thinkHeader} onClick={() => setIsThinkOpen(prev => !prev)}>
                            <span className="flex items-center gap-1.5">
                              <svg
                                ...
                                stroke="currentColor"
                                strokeWidth="2.2"
                                className={`${s.thinkChevron} ${isThinkOpen ? s.thinkChevronOpen : ''}`}
                              >
                                <polyline points="9 18 15 12 9 6" />
                              </svg>
                              <span>思考过程</span>
                            </span>
                            {parsedContent.isThinking && <span className={s.thinkLive}>思考中...</span>}
                          </button>
                          {isThinkOpen && (
                            <div className={s.thinkBody}>
                              <StreamdownMarkdown content={parsedContent.think} />
                            </div>
                          )}
                        </div>
                      )}
                      {!!parsedContent.answer && (
                        <StreamdownMarkdown content={parsedContent.answer} />
                      )}
                    </div>
                  ))}
