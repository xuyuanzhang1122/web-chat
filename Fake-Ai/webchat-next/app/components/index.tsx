...
const Main: FC<IMainProps> = () => {
  ...
  const hideBootLoading = searchParams?.get('show_loading') === '0'
  ...
  */
  ...
  useEffect(() => {
    ...
    const auth = localStorage.getItem('fake-ai-auth')
    if (!auth) {
      setAuthChecked(true)
  ...
  useEffect(() => {
    ...
    if (forcedTheme === 'dark' || forcedTheme === 'light') {
      document.documentElement.style.setProperty('color-scheme', forcedTheme)
    }
    else {
      document.documentElement.style.removeProperty('color-scheme')
    }
  ...
  */
      ...
      async onCompleted(hasError?: boolean) {
        ...
        if (getConversationIdChangeBecauseOfNew()) {
          try {
            ...
            const targetConversationId = safeConversations[0]?.id || tempNewConversationId

            if (targetConversationId) {
              const newItem: any = await generationConversationName(targetConversationId)

              if (safeConversations[0]) {
                const newAllConversations = produce(safeConversations, (draft: any) => {
                  draft[0].name = newItem?.name || draft[0].name
                })
                setConversationList(newAllConversations as any)
              }
              else {
                const { data: refreshedConversations }: any = await fetchConversations()
                if (Array.isArray(refreshedConversations)) {
                  setConversationList(refreshedConversations as any)
                }
              }
            }
          }
          catch (error) {
            console.error('auto rename conversation failed:', error)
          }
        }
        setConversationIdChangeBecauseOfNew(false)
        resetNewConversationInputs()
  ...
  const handleLogout = () => {
    if (typeof window !== 'undefined') {
      localStorage.removeItem('fake-ai-auth')
    }
    router.replace('/login')
  }

  if (!authChecked || !isAuthed) {
    return hideBootLoading ? null : <Loading type='app' />
  }
  ...
  return (
    <div className='bg-slate-100 h-screen overflow-hidden'>
      ...
      />
      <div className="mx-2 my-2 flex rounded-2xl bg-white border border-gray-200 shadow-sm overflow-hidden">
        {/* sidebar */}
        {!isMobile && renderSidebar()}
        {isMobile && isShowSidebar && (
          <div className='fixed inset-0 z-50 bg-gray-900/30' onClick={hideSidebar} >
        ...
        {/* main */}
        <div className='flex-grow flex flex-col h-[calc(100vh_-_5.5rem)] overflow-y-auto bg-gradient-to-b from-white to-slate-50'>
          <ConfigSence
          ...
          {
            hasSetInputs && (
              <div className='relative grow pc:w-[820px] tablet:w-[92%] max-w-full mobile:w-full pb-[180px] mx-auto mb-3.5 px-2 mobile:px-1.5' ref={chatListDomRef}>
                <Chat
                  chatList={chatList}
                  onSend={handleSend}
